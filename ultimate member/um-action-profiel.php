...
function um_profile_content_main( $args ) {
	...
	/*----------ポイント表示の追加要素-------------------*/
	/*デバッグ用
	echo um_profile_id(),'<br>';
	*/

	//UserのIDを取得	
	$profiel_user_id = um_profile_id();

	//UserのIDから権限を取得
	$key = 'user_role';
	$single = true;
	$user_level = get_user_meta($profiel_user_id , $key , $single);

	/*デバッグ用
	echo $user_level;
	*/
	
	//Wordpressの$wpdbを関数内で使用可能にする。
	global $wpdb;
	$myPath = '/home/field-jack/www/catspaw/wp-load.php';
	require_once( $myPath );

	/*アーティストの場合に表示する項目*/
	if($user_level=='um_artist_member'){
		/*デバッグ用
		echo 'Success.';
		*/
		$artist_point = get_user_meta($profiel_user_id , 'artist_point' , $single);
		$artist_last_month_point = get_user_meta($profiel_user_id , 'last_month_point' , $single);
		$artist_this_month_point = get_user_meta($profiel_user_id , 'this_month_point' , $single);
		$artist_last_month_rank = get_user_meta($profiel_user_id , 'last_month_rank' , $single);
		$artist_rank_query = "SELECT `artist_rank` FROM `cats_usermeta` WHERE `meta_key` = 'artist_point' AND `user_id`='" .$profiel_user_id ."';";
		//デバッグ用  echo $artist_rank_query;
		$artist_rank = $wpdb->get_var($artist_rank_query);

		//echo '<h3>総ポイント数 ： ' , $user_point , 'P </h3>';
		echo '<table border="1" width="500" cellspacing="0" cellpadding="5" bordercolor="#333333">
				<tr>
				<th bgcolor="#f0f8ff"><h3>総ポイント<br>', $artist_point,'P</h3></th>
				<th bgcolor="#f0f8ff" width="150"><h4>先月ポイント<br>' , $artist_last_month_point , 'P</h4></th>
				<th bgcolor="#f0f8ff" width="200"><h4>今月ポイント<br>' , $artist_this_month_point , 'P</h4></th>
				</tr>
				</table>';
		...
		echo '<h3>現在全国順位 : ' , $artist_rank , '位</h3>';
		if ($artist_last_month_rank == '順位無し'){
			echo '<h3>先月順位 : ' , $artist_last_month_rank , '</h3>';
		}else{
			echo '<h3>先月順位 : ' , $artist_last_month_rank , '位</h3>';
		}
		...
		/*
		SET @a = 0;
		UPDATE cats_usermeta SET artist_rank = (@a :=@a+1) WHERE `meta_key` = 'artist_point' ORDER BY `meta_value` DESC
		*/
	...
	/*shopの場合に表示する項目*/
	if($user_level=='um_shop_member'){
		$shop_activate_point = get_user_meta($profiel_user_id , 'activate_point' , $single);
		echo '<h4>この店舗から利用された総ポイント数 : ' , $shop_activate_point , 'P</h4>';
	}


	/*ファンユーザの場合に表示する項目*/
	if($user_level=='um_fan_member'){
		$user_point = get_user_meta($profiel_user_id , 'fan_point' , $single);
		$user_last_month_point = get_user_meta($profiel_user_id , 'last_month_point' , $single);
		$user_this_month_point = get_user_meta($profiel_user_id , 'this_month_point' , $single);
		$user_last_month_rank = get_user_meta($profiel_user_id , 'last_month_rank' , $single);


		echo '<table border="1" width="500" cellspacing="0" cellpadding="5" bordercolor="#333333">
				<tr>
				<th bgcolor="#f0f8ff"><h3>総投票ポイント<br>', $user_point,'P</h3></th>
				<th bgcolor="#f0f8ff" width="150"><h4>先月投票ポイント<br>' , $user_last_month_point , 'P</h4></th>
				<th bgcolor="#f0f8ff" width="200"><h4>今月投票ポイント<br>' , $user_this_month_point , 'P</h4></th>
				</tr>
				</table>';
		echo '<h3>先月投票順位 : ' , $user_last_month_rank , '</h3>';
