...
function um_submit_form_register( $args ) {
	...
	$user_id = wp_insert_user( $userdata );
       ...
       //------------ポイント追加要素---------------//
        ...
        /*アーティストの場合追加する項目*/
        if($user_role=='um_artist_member'){
        	add_user_meta($user_id , 'artist_point' , 0);
        	add_user_meta($user_id , 'user_role' , $user_role);
        	add_user_meta($user_id , 'last_month_point' , 0);
        	add_user_meta($user_id , 'this_month_point' , 0);
        	add_user_meta($user_id , 'last_month_rank' , '順位無し');
        	add_user_meta($user_id , 'vote_rireki' , NULL);
        }

        /*店舗の場合追加する項目*/
        if($user_role=='um_shop_member'){
        	add_user_meta($user_id , 'user_role' , $user_role);
        	add_user_meta($user_id , 'register_card' , 0); 
        	add_user_meta($user_id , 'activate_point' , 0);
        }

        /*ファンユーザーの場合追加する項目*/
        if($user_role=='um_fan_member'){
        	add_user_meta($user_id , 'user_role' , $user_role);
        	add_user_meta($user_id , 'fan_point' , 0);
        	add_user_meta($user_id , 'last_month_rank' , '順位無し');
        	add_user_meta($user_id , 'last_month_point' , 0);
        	add_user_meta($user_id , 'this_month_point' , 0);
        	add_user_meta($user_id , 'rireki' , NULL);
        	add_user_meta($user_id , 'okiniiri' , NULL);
        	add_user_meta($user_id , 'vote_rireki' , NULL);
        }
